...
void RangeSlider::wheelEvent(QWheelEvent *event)
{
    ...
    int steps = numDegrees.y() / 15;  // Vertical scrolling

    if (steps != 0) {  // Ensure there's actual scrolling
        if (steps < 0) {
            // Scrolling down (negative direction)
            ...
            // Scrolling up (positive direction)
            if (keys.testFlag(Qt::ShiftModifier)) {
                if (keys.testFlag(Qt::ControlModifier)) {
                    value += 20;
                } else {
